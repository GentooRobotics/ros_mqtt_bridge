...
class ROSBridge():
    def __init__(self, 
        ...
        self.sleep_time = 1. / (self.loop_rate) # default loop rate
    ...
    def ros_callback(self, msg, converter, ros_topic):
        ...
        mqtt_topic: str = self.cfg["ros2mqtt"]["topic2topic"][ros_topic]["mqtt_topic_name"]
    ...
    def initialize_publish_rate(self, cfg: dict, publisher_key: str):
        if "rate" in cfg:
            ...
            if given_rate > self.loop_rate:
                rospy.logwarn(f"[ROS Bridge] rate given {cfg['rate']} is higher than loop rate, setting rate to loop rate {self.loop_rate}")
                self.publisher_rate[publisher_key] = 1/(1/self.loop_rate - self.sleep_time/2)
            else:
                self.publisher_rate[publisher_key] = 1/(1/given_rate  - self.sleep_time/2)
        else:
            self.publisher_rate[publisher_key] =1/(1/self.loop_rate - self.sleep_time/2)
    ...
    def create_ros_service_clients(self):
        if "mqtt2ros" not in self.cfg or "topic2service" not in self.cfg["mqtt2ros"] or not self.cfg["mqtt2ros"]["topic2service"]:
        ...
        for mqtt_request_topic in self.cfg["mqtt2ros"]["topic2service"]:
            topic2service: dict[str, str] = self.cfg["mqtt2ros"]["topic2service"][mqtt_request_topic]
            ...
            rospy.loginfo(f"[ROS Bridge] bridge <-  [MQTT][topic]  {mqtt_request_topic}")
    ...
    def timer_callback(self, _):
        ...
        for mqtt_receive_topic_name, mqtt2ros_item in self.mqtt2ros_tasks.items():
            ...
            if(mqtt2ros_item.last_received_time <= mqtt2ros_item.last_published_time): # no new message
                continue

            if (mqtt2ros_item.command == "topic2topic"):
                if (current_time - mqtt2ros_item.last_published_time < 1. / self.publisher_rate[mqtt_receive_topic_name]):
                    continue

                ros_msg_typename: str = self.cfg["mqtt2ros"]["topic2topic"][mqtt_receive_topic_name]["ros_type"]
                self.ros_publishers[mqtt_receive_topic_name].publish(convert_dictionary_to_ros_message(hydra.utils.get_class(ros_msg_typename), json.loads(mqtt2ros_item.payload)))
                ...
                def service_call(self: ROSBridge, mqtt_receive_topic_name: str, mqtt2ros_item: MQTT2ROSItem, ros_service: str):
                    try:
                    ...
                    try:
                        request = convert_dictionary_to_ros_message(hydra.utils.get_class(ros_srv_typename+"Request"), json.loads(mqtt2ros_item.payload))
                    ...
                    self.ros2mqtt_tasks[mqtt_response_topic_name].converter = "primitive"
                    self.ros2mqtt_tasks[mqtt_response_topic_name].last_received_time = time.time()
                KillableTimer(0.0, service_call, args=(self, mqtt_receive_topic_name, mqtt2ros_item, ros_service)).start()